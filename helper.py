...
# 检测是否存在图片或poco
def exists_any(obj, isReturnObj = False):
    
    if (isReturnObj == False):
        return exists_any(obj, isReturnObj = True)[0]
    ...
    if (type(obj) == UIObjectProxy):
        try:
            if (obj.exists()):
                return [position_to_absolute(obj.get_position()), obj]
            else:
                return [False, None]
        except:
            return [False, None]
    ...
    if (type(obj) == list):
        for item in obj:
            result = exists_any(item, isReturnObj = True)
            if (result[0] != False):
                return result
        return [False, None]
