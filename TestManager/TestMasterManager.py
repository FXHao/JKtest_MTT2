...
class MasterManager:
    ...
    def __init__(self):
        self.poco = AndroidUiautomationPoco(screenshot_each_action = True)
    ...
    def config(self, config_file = ""):
        ...
        self.config_prxocess("process.CSV")
    ...
    # [Helper] 配置流程
    def config_prxocess(self, process_control_file = None):
        ...
        with open(_project_root + process_control_file,"r") as csvParse:
            for line in csvParse:
                ...
                if (test_name != ""):
                    ...
                    curID += 1
                    #print("0000000000000000000000000000000000",test_name,case.name)
                else:
                    continue
    ...
    def run_test(self):
        for case in self.TestCaseList:
            ...
            try:
                try:
                    case.test_function(case.args)
                except TypeError:
                    self.Reporter.report(case.name + "测试不接受参数.")
                    case.test_function()
                case.onTestSuccess()
            except:
                case.onTestFailed()
                traceback.print_exc()
                snapshot(msg=traceback.format_exc())
                if (case.name == "Guide"):
                    self.Reporter.report("Guide failed, ending.")
                    return
